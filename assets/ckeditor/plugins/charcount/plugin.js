...
function calculate(editor) {
    ...
    if (editor.config.maxLength != null) {
        ...
        if (editor.config.maxLength == 0) {
            ...
            diff = editor.config.maxLength - plainText.length;
            buttonLabel.innerHTML = diff;
        }
        ...
        if (editor.config.maxWords == 0) {
            ...
            diff =  editor.config.maxWords - wordCount(plainText);

            if(diff > editor.config.maxWords){
...
/**
 * Trims off the excess words from the text and returns the new text
 * @param {String} text, {Integer} max
 * @return {String} text
 *
 */
