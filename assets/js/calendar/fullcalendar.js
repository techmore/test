...
var Grid = fc.Grid = RowRenderer.extend({
		...
		return '' +
			'<div class="fc-row ' + this.view.widgetHeaderClass + '">' +
				'<table>' +
					...
					'</thead>' +
					'<tr style="display: none;"><td>1</td><td>2</td><td>3</td><td>4</td><td>5</td><td>6</td><td>7</td></tr>' +
				'</table>' +
			'</div>';
	},
	...
	headCellHtml: function(cell) {
		...
		return '' +
			'<th scope="col" role="columnheader" class="fc-day-header ' + view.widgetHeaderClass + ' fc-' + dayIDs[date.day()] + '">' +
...
Grid.mixin({
		...
		$.each(
			{
				...
				focus:function(seg, ev){
					_this.triggerSegFocus(seg, ev);
				},
				blur:function(seg, ev){
					_this.triggerSegBlur(seg, ev);
				}
			},
			function(name, func) {
	...
	triggerSegFocus: function(seg, ev) {
		if (!this.mousedOverSeg) {
			this.mousedOverSeg = seg;
			this.view.trigger('eventFocus', seg.el[0], seg.event, ev);
		}
	},

	triggerSegBlur: function(seg, ev) {
		ev = ev || {}; // if given no args, make a mock mouse event

		if (this.mousedOverSeg) {
			seg = seg || this.mousedOverSeg; // if given no args, use the currently moused-over segment
			this.mousedOverSeg = null;
			this.view.trigger('eventBlur', seg.el[0], seg.event, ev);
		}
	},
...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		var eventTitle = event.official;
		var eventSchool = (event.school==null) ? "state" : event.school;
		var eventStarts = event.start;
		var eventEnds = event.end;
		...
		return '<a title="'+ finalString +'" tabindex="0" class="' + classes.join(' ') + '"' +
				(event.url ?
...
TimeGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		return '<a tabindex="0" class="' + classes.join(' ') + '"' +
			(event.url ?
				' href="' + htmlEscape(event.url) + '"' :
				' href="#" '
				) +
				...
				$.each(this.split(','), function(j, buttonName) {
					...
					if (buttonName == 'title') {
						...
						if (buttonClick) {
							...
							//var btnLabel = (buttonName)
							button = $( // type="button" so that it doesn't submit a form
								'<button  aria-label="'+buttonName+'" type="button" class="' + classes.join(' ') + '">' +
									innerHtml +
								'</button>'
								)
								.click(function() {
...
var BasicView = fcViews.basic = View.extend({
	...
	numberCellHtml: function(cell) {
		...
		return '' +
			'<td tabindex="0" class="' + classes.join(' ') + '" data-date="' + date.format() + '">' +
				date.date() +
			'</td>';
	},
