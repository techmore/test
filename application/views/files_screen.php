...
<div id="file-upload-dialog" title="File Upload">
    <form enctype="multipart/form-data" method="post" id="upload_form" action="<?php echo(base_url('/files/upload')); ?>">
        <p>&nbsp;</p>
        <p>
            <input type="file" name="userfile[]" id="userfile" multiple accept="image/*, application/msword,application/vnd.ms-excel, application/excel, application/vnd.openxmlformats-officedocument.wordprocessingml.document, , .png, .gif, .jpeg, .doc, .docx, .xls, .xlsx, .pdf, .ppt, .pptx, .txt" required/>
        </p>
    ...
    $(document).ready(function(){
        ...
        var deleteFileDialog = $( "#deleteFileDialog" ).dialog({
            ...
            buttons: {
                "Delete": function(){
                    $(this).dialog('close');
                    location.href="<?php echo(base_url()); ?>files/delete/" + selectedId;
                },
                Cancel: function() {
                    $( this ).dialog( "close" );
                    return false;
